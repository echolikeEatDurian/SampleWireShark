...
{
    ui->setupUi(this);
    statusBar()->showMessage("welcome to simple_wireshark!");
    // set the toolbar,push the icon into the toolbar
    ...
    // initialization,data.clear not swap,just clear the 'data' of data
    ...
    //before capturing ,we set the filter
    connect(ui->lineEdit_2, &QLineEdit::textChanged, this, [=]() {
        QString filterStr = ui->lineEdit_2->text();
        qDebug() << " QString filterStr = ui->lineEdit_2->text();:"
                 <<filterStr;
        thread->setFilterStr(filterStr);
        qDebug() << "thread->filterStr:"
                 <<thread->getFilterStr();
    });
    ...
    //when isStart is false,we can do this.
    ...
    if(rowNumber == row || row < 0){
        ...
        if(packageType == ARP){
            ...
            if(packageType == TCP || packageType == TLS || packageType == SSL){
                ...
                dataLengthofIp -= (rawLength * 4);// 32 * 4
