...
void Capture::run(){
    ...
    while(true){
        if(isDone)
            break;
        //过滤器
        qDebug() << "这个线程的过滤器是:" << this->filterStr ;
        struct bpf_program filter;
        QByteArray byteArray = this->filterStr.toUtf8(); // 将QString转换为UTF-8格式的字节数组
         const char* filterExpression = byteArray.constData(); // 获取字节数组的const char*指针LF
        u_int netmask = inet_addr("255.255.255.0");


        pcap_compile(pointer,&filter,filterExpression,1,netmask);
        ...
        QString info = "";//get data infomations
...
*/
    IP_HEADER* ip;
    //这个偏移量 14 表示在以太网帧中跳过以太网头部（14 字节
    ip = (IP_HEADER*)(pkt_content + 14);
    int protocol = ip->protocol;
    //用于计算 IP 头部的长度（以32位字为单位）
...
void Capture::setFilterStr(QString filterstr)
{
    this->filterStr = filterstr;
}

QString Capture::getFilterStr()
{
